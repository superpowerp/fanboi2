        ...
        <div class="container">
            <div class="post">
                <div class="panel__item panel__item--inset">
                    <a class="post__item ${'post__item--inversse' if post.bumped else 'post__item--shallow'}" href="${request.route_path('topic_scoped', board=post.topic.board.slug, topic=post.topic.id, query=post.number)}" data-topic-quick-reply="${post.number}">${post.number}</a>
